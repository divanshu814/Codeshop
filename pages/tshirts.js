    ...
    <div className="flex flex-wrap -m-4 justify-center">
{
  Object.keys(products).map((item)=>{
     ...
     return <Link key={products[item]._id} href={`product/${products[item].slug}`} legacyBehavior>
      <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-md m-5">
        <a className="block relative rounded overflow-hidden">
          <img alt="ecommerce" className="m-auto md:m-0 h-[40vh] w-[30vh] block" src={products[item].img}/>
        </a>
...
export async function getServerSideProps(context){
  ...
  const check=  await mongoose.connect(process.env.MONGO_URI)
